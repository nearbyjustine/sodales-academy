...
(`queries.ts`/`mutations.ts`/`authz.ts`), `src/lib/session.ts`, `src/db/` itself (`seed.ts`, run
once to promote `ADMIN_EMAIL` to admin, and `seed-tracks.ts`, which seeds the tracks catalog
content and has not yet been run in production), and `src/app/actions/verify-invite-code.ts` (the
invite-code Server Action, which has to check the DB before Google OAuth can even be attempted —
see below). Keep it that way; verify with `grep -rn "@/db" src` before assuming otherwise, since
this list is a description of current reality, not an enforced boundary.
...
- **Tracks are admin-managed, not instructor-managed.** Every track write (create/update/publish/
  unpublish/delete) calls `requireRole("admin")` — deliberately narrower than the course mutations'
  `requireRole("instructor", "admin")` — because a track can contain courses owned by several
  different instructors and `assertCanManageCourse`'s single-owner check has no equivalent for it.
- **Enrolling in a track fans out to per-course `enrollment` rows; nothing is recorded against the
  track itself.** `enrollInTrack` writes one `enrollment` row per course the track links (reusing
  the same enrollment path a direct course enrollment would), which is exactly why deleting a track
  destroys no learner progress — the progress lives on the courses, not the track. Do not
  "optimise" this into a `track_enrollment` table without re-deciding that guarantee.
...
pnpm db:seed:tracks  # seed the tracks catalog content (idempotent) — dotenv -e .env.local -- tsx ... src/db/seed-tracks-cli.ts
```
...
| `src/db/schema/` | `user.ts` (`user_profile`), `course.ts` (`course`/`course_module`/`lesson`), `enrollment.ts` (`enrollment`/`lesson_progress`), `invite.ts` (`invite_code`), `track.ts` (`track`/`track_course`) — UUID PKs, `snake_case`, `created_at`/`updated_at` on mutable tables |
...
| `src/db/seed-tracks.ts` / `seed-tracks-cli.ts` | Seeds the tracks catalog content, idempotent — not yet run in production |
...
| `src/lib/content/queries.ts` | Reads — `getCourses`/`getCourseBySlug`/`getCourseBySlugForAdmin`/`getLesson`/`getCatalogStats`/`getAllCourses`/`isEnrolled`/`getCompletedLessonIds`/`getTracks`/`getTrackBySlug`/`getTracksForAdmin`/`getTracksForCourse`/`getFullyEnrolledTrackSlugs` (the last is a single bounded query so the dashboard can flag "fully enrolled" tracks without fanning out over the whole catalog) |
| `src/lib/content/mutations.ts` | `"use server"` writes — `createCourse`/`updateCourse`/`publishCourse`/`unpublishCourse`/`deleteCourse`/`enrollInCourse`/`toggleLessonComplete`/`listInstructors`/`createTrack`/`updateTrack`/`publishTrack`/`unpublishTrack`/`deleteTrack`/`enrollInTrack` |
...
| `src/lib/track-progress.ts` | Pure helpers deriving a learner's per-track progress from course/enrollment data — no separate track-progress table |
| `src/lib/validation.ts` | zod schemas (`courseInputSchema`, `trackInputSchema`, `signUpSchema`, `signInSchema`) — `courseInputSchema` carries `instructorUserId`, server-overwritten when the submitter is an instructor |
...
| `src/components/track/` | Track map, track row, breadcrumb, progress card, enroll button |
...
| `src/app/(site)/` | Public + learner routes — home, `/courses`, `/dashboard`, `/learn`, `/tracks` (has `SiteHeader`/`SiteFooter`) |
| `src/app/(auth)/` | `/login`, `/sign-up` — split-screen layout, no site chrome |
| `src/app/admin/` | `/admin/*` — sidebar shell, gated by `requireRole` in its layout AND in each page (cheap, idempotent, React-cached); includes `/admin/tracks` |

## Outstanding / deferred

- **Single-use invite codes, locked progression, and points/streaks/badges are specced follow-ups,
  not indefinite deferrals.** The journey spec (`docs/superpowers/specs/2026-09-05-academy-journey-
  design.md` §1) names all three as the next specs in an explicit order — Seats & access
  (single-use codes bound to one redeemer, admin minting), then Locked progression (courses/lessons
  unlock in sequence within a track, needs tracks to exist first), then Points/streaks/badges
  (retention mechanics over real write paths). None are implemented yet; today one invite code still
  admits unlimited people, and this spec explicitly renders a track's shape without locking access
  to it.
- **Invite code rotation UX.** Codes live in the `invite_code` DB table so an admin *can* rotate one
  ...
  direct DB access. Folds into the Seats & access spec above.
...
- **Progression locking, an interactive "find your track" picker, and proof/testimonials/student
  work on the pitch** are explicitly out of scope for the journey spec itself (§3, §8) — locking is
  deferred to its own spec because it's a product decision, not a rendering detail.
...
**The Academy sells seats to customers** (journey spec §2 — a breaking, documented change from the
earlier positioning). An invite code is a receipt for a paid seat, sold and fulfilled manually
off-site (payment happens outside the codebase; there is no payment provider here, and
`docs/02-academy.md`'s deferral of payments still holds) — an admin mints a code and sends it to
the buyer, who redeems it via `verify-invite-code.ts`. Today one code still admits unlimited
people; single-use, per-person codes are a specced follow-up (Seats & access), not yet
implemented. The public catalog remains world-readable and still doubles as Agency proof-of-work,
but that is now a side effect of the product being public, not the purpose of building it.
